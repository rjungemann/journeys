    ...
    <p>
      ...
      {commaSeparateComponents(
        otherEntities.map((entity) => {
          return (
            <a key={entity.name} onClick={handleEntityFn(entity)}>
              {entity.title}
            </a>
          )
        }),
...
export const RoomHostileEntitiesView = () => {
  const { state, dispatch } = useGame()
  const openlyHostileEntities = findRoom(state)(state.roomName)
  ...
  const alliedHostileEntities = findRoom(state)(state.roomName)
    .entities.map((entityName) => findEntity(state)(entityName))
    .filter((entity) =>
